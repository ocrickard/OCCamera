...
- (void)_init
{
    ...
    defaultOptions          = @{ DIYAVSettingFlash              : @false,
                                 ...
                                 DIYAVSettingVideoPreset        : AVCaptureSessionPresetHigh,
...
- (BOOL)ready {
    if(self.videoInput.device.isAdjustingFocus) {
        return NO;
    } else {
        return YES;
    }
...
- (void)focusAtPoint:(CGPoint)point inFrame:(CGRect)frame
{
    if (self.videoInput.device.isFocusPointOfInterestSupported && [self.videoInput.device isFocusModeSupported:AVCaptureFocusModeAutoFocus]) {
        ...
        NSLog(@"Error, device is not capable of focusing on the point of interest, or is not capable of autofocus");
    }
}

- (void)focusAndExposeAtPoint:(CGPoint)point inFrame:(CGRect)frame {
    if (self.videoInput.device.isFocusPointOfInterestSupported && [self.videoInput.device isFocusModeSupported:AVCaptureFocusModeAutoFocus] && self.videoInput.device.isExposurePointOfInterestSupported && [self.videoInput.device isExposureModeSupported:AVCaptureExposureModeContinuousAutoExposure]) {
        ...
        if ([self.videoInput.device lockForConfiguration:&error]) {
            self.videoInput.device.focusPointOfInterest = focusPoint;
            self.videoInput.device.focusMode = AVCaptureFocusModeAutoFocus;
            self.videoInput.device.exposurePointOfInterest = focusPoint;
...
- (void)exposeAtPoint:(CGPoint)point inFrame:(CGRect)frame
{
...
- (void)capturePhoto
{
    if (self.session != nil) {
        ...
        if ([[self.options valueForKey:DIYAVSettingOrientationForce] boolValue]) {
            ...
            stillImageConnection.videoOrientation = [DIYAVUtilities getAVPictureCaptureOrientationFromDeviceOrientation];
        }
...
- (void)captureVideoStart
{
    if (self.session != nil) {
        ...
        if ([videoConnection isVideoOrientationSupported] && ![[self.options valueForKey:DIYAVSettingOrientationForce] boolValue]) {
            [videoConnection setVideoOrientation:[DIYAVUtilities getAVVideoCaptureOrientationFromDeviceOrientation]];
